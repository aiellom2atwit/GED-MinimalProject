...
typedef std::tuple<CTransform, CLifespan, CInput, CBoundingBox, CGravity, CAnimation, CState> ComponentsTuple;
...
class Entity{
    ...
    private:
        ...
        ComponentsTuple components=std::make_tuple(CTransform(), CLifespan(5), CInput(), CBoundingBox(), CGravity(), CAnimation(), CState()); /* Data structure that contains all possible entity components, ensures that only one of each can exist*/
